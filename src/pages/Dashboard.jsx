...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        const openTodos = todos?.filter(
            t => t.assignedTo.includes(currentUser.id) && !t.completedBy.includes(currentUser.id)
        ) || [];
        ...
        const upcomingTrainings = trainingRequests?.filter(t => {
            ...
            return date > today && (t.status === 'pending' || t.status === 'approved');
        }).sort((a, b) => {
        ...
        const dueToday = openTodos.filter(t => t.dueDateKey === todayKey);
                            ...
                            <div className="space-y-4">
                                {recentUnreadNotifications.slice(0, 5).map(notification => ( // Show up to 5 recent notifications
                                    <div key={notification.id} className="flex items-start gap-3 p-3 rounded-xl hover:bg-gray-50 transition-colors">
                                        <div className={`p-2 rounded-lg shrink-0 ${notification.type === 'info' ? 'bg-blue-50 text-blue-600' :
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-gray-500 mt-1">
                                                {formatDatePretty(new Date(notification.createdAt))}
                                            </p>
                                        </div>
                                    </div>
                                ))}
