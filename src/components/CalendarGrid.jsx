            ...
            <div className="grid grid-cols-7 grid-rows-6 flex-1 min-h-0 bg-gray-50/30">
                {daysArray.map((d, idx) => {
                    ...
                    const myTasks = todos.filter(
                        t => t.dueDateKey === dKey && !t.completedBy.includes(currentUser?.id)
                    );

                    // Meetings - Scheduled Date
                    const myMeetings = meetingRequests.filter(
                        m => m.scheduledDateKey === dKey && m.status === 'approved'
                    );
                    ...
                    if (myTraining) {
                        const trainingDetail =
                            myTraining.status === 'pending' ? 'Solicitud pendiente' :
                                myTraining.status === 'rescheduled' ? 'Formación reprogramada' :
                    ...
                    myMeetings.forEach(meeting => {
                        badges.push({
                            type: 'meeting',
                            label: meeting.scheduledTime ? `${meeting.scheduledTime} Reunión` : 'Reunión',
