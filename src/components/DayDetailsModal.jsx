 ...
 */
export default function DayDetailsModal({ date, events, onClose }) {
                            ...
                            {events.absences.length > 0 && (
                                <div className="space-y-3">
                                    ...
                                    {events.absences.map((absence) => {
                                        const user = USERS.find(u => u.id === absence.createdBy);
                                        const isOtherUser = events.isAdmin && absence.createdBy !== currentUser?.id;
                                        return (
                                            <div key={absence.id} className="p-4 rounded-xl bg-purple-50 border border-purple-200">
                                                <div className="flex items-center justify-between mb-2">
                                                    <div className="flex items-center gap-2">
                                                        <span className="text-sm font-bold text-gray-900">
                                                            {absence.type === 'vacation' ? 'Vacaciones' : 'Permiso especial'}
                                                        </span>
                                                        {isOtherUser && (
                            ...
                            {events.trainings.length > 0 && (
                                <div className="space-y-3">
                                    ...
                                    {events.trainings.map((training) => {
                                        ...
                                        const isOtherUser = (events.isAdmin || events.isTrainingManager) && training.userId !== currentUser?.id;
                                        const isAdminView = events.isAdmin && training.userId !== currentUser?.id;
                                        const isTrainingManagerView = events.isTrainingManager && !events.isAdmin && training.userId !== currentUser?.id;
                                        return (
                                            <div key={training.id} className="p-4 rounded-xl bg-blue-50 border border-blue-200">
                                                <div className="flex items-center justify-between mb-2">
                                                    ...
                                                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${training.status === 'approved' ? 'bg-green-100 text-green-700' :
                                                            training.status === 'pending' ? 'bg-amber-100 text-amber-700' :
                                                                training.status === 'rescheduled' ? 'bg-purple-100 text-purple-700' :
                                                                    'bg-gray-100 text-gray-700'
                                                        }`}>
                                                        {training.status === 'approved' ? 'Aprobado' :
                            ...
                            {events.meetings.length > 0 && (
                                <div className="space-y-3">
                                    ...
                                    {events.meetings.map((meeting) => {
                                        const user = USERS.find(u => u.id === meeting.createdBy);
                                        const isOtherUser = events.isAdmin && meeting.createdBy !== currentUser?.id;
                                        return (
                                ...
                                <div className="space-y-3">
                                    ...
                                    <div className="space-y-2">
                                        {events.tasks.map((task) => {
                                            ...
                                            const isOtherUser = events.isAdmin && task.createdBy !== currentUser?.id;
                                            return (
